...
import { LIKE,UPDATE_EVENT,CREATE_EVENT,FETCH_EVENTS,DELETE_EVENT ,FETCH_EVENT,FETCH_EVENTS_BY_SEARCH,START_LOADING,END_LOADING,COMMENT_EVENT,FETCH_EVENTS_BY_CREATOR,ATTEND_EVENT} from '../constants/actionTypes.js';
const EventReducer= ( state= {isLoading:true,events:[]},action)=>{
  switch (action.type) {
    ...
    case FETCH_EVENT:
     ...
     case ATTEND_EVENT:
      return {...state,event:state.events.map((event) => (event._id === action.payload._id ? action.payload : event))};
